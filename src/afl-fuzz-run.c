...
u8 __attribute__((hot)) common_fuzz_stuff(afl_state_t *afl, u8 *out_buf,
  ...
  if( 0 != strcmp(afl->stage_name, "calibration") && 0 != strcmp(afl->stage_name, "colorization") && 0 != strncmp(afl->stage_name, "trim", 4) ) {
    ...
    // if (afl->fsrv.path_trace_bits[0] <= 0) {
    //     printf("cyherr: afl->fsrv.path_trace_bits[0] = %u\n", afl->fsrv.path_trace_bits[0]);
    //     printf("========== out_buf start ============\n");
    //     const unsigned char *byte = (const unsigned char *)out_buf; // 转换为字节指针
    //     for (size_t i = 0; i < len; i++) {
    //         printf("%c", byte[i]); // 打印每个字节为字符
    //     }
    //     printf("\n");
    //     printf("========== out_buf end ============\n");
    //     assert(0);
